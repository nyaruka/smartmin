...
"""),
            ...
            MockMigration(operations=[
                RunSQL("CREATE OR REPLACE FUNCTION a_func(i INTEGER, s TEXT) RETURNS integer AS $$ BEGIN RETURN i + 1; END; $$ LANGUAGE plpgsql;"),

                # different function because it has different parameters
                RunSQL("CREATE OR REPLACE FUNCTION a_func(s text, integer) RETURNS text AS $$ BEGIN RETURN UPPER(s); END; $$ LANGUAGE plpgsql;"),
        ...
        mock_write_dump.assert_has_calls([
            call('indexes', [
                SqlObjectOperation("CREATE INDEX a_test ON foo(bar);", sql_type=SqlType.INDEX, obj_name="a_test", is_create=True),
                SqlObjectOperation("CREATE INDEX test_1 ON foo(bar);", sql_type=SqlType.INDEX, obj_name="test_1", is_create=True),
                SqlObjectOperation("create unique index test_3 on foo(bar);", sql_type=SqlType.INDEX, obj_name="test_3", is_create=True),
            ], 'sql'),
            call('functions', [
                SqlObjectOperation(
